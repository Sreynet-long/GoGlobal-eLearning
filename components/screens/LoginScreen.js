...
import { t } from "../../lang";
import { GET_USER_BY_ID, MOBILE_LOGIN, MOBILE_REGISTER_ACCOUNT } from "../../schema/login";
  ...
  const [loginMutation] = useMutation(MOBILE_LOGIN, {
    onCompleted: async (data) => {
      if (data?.mobileLogin?.status) {
        ...
        await fetchUser();
        resetForm();
      } else setError(data?.mobileLogin?.message?.[`message${language.toUpperCase()}`]);
    },
  ...
  const [signupMutation] = useMutation(MOBILE_REGISTER_ACCOUNT, {
    onCompleted: async (data) => {
      if (data?.mobileRegisterAccount?.status) {
        ...
        await fetchUser();
        resetForm();
      } else setError(data?.mobileRegisterAccount?.message?.[`message${language.toUpperCase()}`]);
    },
    onError: (err) => setError(err.message),
  });
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Card style={styles.authCard}>
          <Card.Content>
            <Text style={styles.title}>{isSignUp ? t("create_account", language) : t("log_in", language)}</Text>
            ...
            {isSignUp && (
              <>
                <View style={styles.row}>
                  <TextInput label={t("first_name", language)} mode="outlined" value={firstName} onChangeText={setFirstName} style={styles.flex} />
                  <TextInput label={t("last_name", language)} mode="outlined" value={lastName} onChangeText={setLastName} style={styles.flex} />
                </View>

                <TextInput label={t("phone", language)} mode="outlined" value={phone} onChangeText={setPhone} style={styles.input} />
                ...
                <View style={styles.checkboxRow}>
                  <Checkbox status={agreeTerms ? "checked" : "unchecked"} onPress={() => setAgreeTerms(!agreeTerms)} />
            ...
            <TextInput label={t("email", language)} mode="outlined" value={email} onChangeText={setEmail} style={styles.input} />
            <TextInput label={t("password", language)} mode="outlined" secureTextEntry value={password} onChangeText={setPassword} style={styles.input} />

            <Button mode="contained" onPress={handleAuth} style={{ backgroundColor: "#25375A" }}>{isSignUp ? t("sign_up", language) : t("log_in", language)}</Button>
            {!isSignUp && <Button onPress={() => router.push("/auth/forgot-password")} textColor="#25375A">{t("forgot_password", language)}</Button>}
            <Button onPress={toggleForm}>{isSignUp ? t("already_have_account", language) : t("dont_have_account", language)}</Button>
          </Card.Content>
        </Card>
      </ScrollView>
    </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  screenContainer: { flex: 1, backgroundColor: "#f7f7f7ff" },
