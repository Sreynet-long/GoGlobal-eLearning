...
export default function CoursePlayerScreen() {
  ...
  const [activeTab, setActiveTab] = useState(TAB_KEYS.CONTENT);
  const [isPlaying, setIsPlaying] = useState(false);
  const [selectedVideo, setSelectedVideo] = useState(null);

  // const TABS = [t("course_content",language), t("overview",language)];
  const TABS = [
    {
      key: TAB_KEYS.CONTENT,
      label: t("course_content", language),
    },
    {
  ...
  const renderOverview = () => (
    ...
    >
      <View style={styles.card}>
        <Text style={styles.cardTitle}>{t("about_this_course", language)}</Text>
        <InfoRow
          icon="tag-outline"
        ...
        <InfoRow
          icon="clock-outline"
          label={t("total_hours", language)}
          value={includes?.number_of_hours}
        />
        <InfoRow
          icon="file-document-outline"
          label={t("quizzes", language)}
          value={includes?.number_quizzes}
        />
        <View>
          <IncludeItem
            text={`${includes?.number_of_downloadable_resources} ${t(
          ...
          />
          <IncludeItem
            text={`${includes?.number_of_projects_practices} ${t(
              "projects_practices",
              language
            )}`}
          ...
          {includes?.is_full_lifetime_access && (
            <IncludeItem
              text={t("full_lifetime_access", language)}
              icon="infinity"
            />
          )}
          {includes?.has_certificate_of_completion && (
            <IncludeItem
              text={t("certificate_of_completion", language)}
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>
          {t("what_you_will_learn", language)}
        </Text>
        <ParagraphList
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>
          {t("who_this_course_is_for", language)}
        </Text>
        <ParagraphList
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>{t("requirements", language)}</Text>
        <ParagraphList text={course?.requirements} icon="tools" />
      </View>

      <View style={styles.card}>
        <Text style={styles.cardTitle}>{t("description", language)}</Text>
